...
test.describe("public estate journeys", () => {
  ...
  test("registry data reaches the Lab system map", async ({ page, request }) => {
    ...
    await expect(page.locator("#system-map-statusline")).not.toContainText(
      /contacting registry/i,
      { timeout: 20_000 },
    );
    await expect(page.locator("#system-map-list > *").first()).toBeVisible({
      timeout: 20_000,
    });
  });

  test("estate search returns provenance or an honest sleep state", async ({ request }) => {
    ...
    if (response.status() === 503) {
      ...
      expect(serialized).toMatch(/down|offline|degraded|stale|unreachable|false/);
      return;
    }
  ...
  test("local AI services report honest availability", async ({ request }) => {
    ...
    if (ramone.awake) {
      const corpus = await jsonOrFail(
        await request.get(`${CORPUS}/health`),
        "corpus health",
      );
  ...
  test("public status surface renders service state", async ({ page }) => {
    ...
    await expect(page.locator("body")).toContainText(/Atlas Systems/i);
